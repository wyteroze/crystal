...
pub fn build(b: *std.Build) !void {
    ...
    const zlib_enabled = b.option(bool, "zlib", "Enable bundled zlib") orelse true;
    ...
    inline for (comptime std.meta.declarations(sources.libraries)) |ext_lib| {
        const is_zlib = comptime std.mem.eql(u8, ext_lib.name, "zlib");
        if (!is_zlib or zlib_enabled) {
            lib.root_module.addCSourceFiles(.{
                .root = assimp.path(""),
...
const unsupported_formats = [_][]const u8{
    ...
    "USD", // Requires tinyusdz, which isn't vendored
};
